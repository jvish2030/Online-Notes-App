...
$sql = "SELECT user_id FROM forgot_password WHERE
`user_id` = '$user_id' AND `r_key` = '$key' AND `status` = 'pending'";
...
$sql = "UPDATE forgot_password SET status='used' WHERE `r_key`='$key' AND `user_id`='$user_id'";
$result = mysqli_query($link, $sql);
if(!$result){
